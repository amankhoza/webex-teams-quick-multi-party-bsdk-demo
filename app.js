...
function switcheroo() {
  ...
  currentStream = (currentStream + 1) % streamz.length;
  showCurrentStream()
}

function showCurrentStream() {
  if (streamz[currentStream] ===  null)
    document.getElementById('remote-view-video').srcObject = null;
  else
    document.getElementById('remote-view-video').srcObject = streamz[currentStream].stream;
}
